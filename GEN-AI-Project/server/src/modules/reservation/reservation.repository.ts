...
export class ReservationRepository {
  ...
  async getReservedSlots(roomNumber: string, date: string) {
    ...
    return reservations.map(reservation => {
      ...
      const startHours = reservation.startDatetime.getHours().toString().padStart(2, '0');
      const startMinutes = reservation.startDatetime.getMinutes().toString().padStart(2, '0');
      const endHours = reservation.endDatetime.getHours().toString().padStart(2, '0');
      const endMinutes = reservation.endDatetime.getMinutes().toString().padStart(2, '0');
      
      const startTime = `${startHours}:${startMinutes}`;
      const endTime = `${endHours}:${endMinutes}`;
      
      console.log(`Backend debug - Converting reservation: ${reservation.startDatetime.toISOString()} -> ${startTime}-${endTime}`);
