...
exports.checkEnvironmentVariables = void 0;
/**
 *
...
const checkEnvironmentVariables = (variables) => {
    if (variables.length === 0) {
        throw new Error('The array is empty. Please add some environment variables into the array.');
    }
    for (const variable of variables) {
        if (!(variable in process.env) || !process.env[variable] || process.env[variable]?.trim() === '') {
            throw new Error(`${variable} is missing or empty in .env file. Check ${variable} and its value.`);
        }
    }
};
