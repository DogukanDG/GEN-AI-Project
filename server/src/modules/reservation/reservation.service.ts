...
export class ReservationService {
  ...
  async createReservation(data: CreateReservationDto) {
    ...
    if (!isAvailable) {
      throw new HttpError(409, 'Room is not available for the selected time slot');
    }
    ...
    try {
      const confirmationMessage = await geminiService.generateBookingConfirmation(reservation);
      return {
        reservation,
        confirmationMessage
      };
  ...
  async cancelReservation(id: number) {
    ...
    return await reservationRepository.update(id, { bookingStatus: 'cancelled' });
  }
  ...
  async searchRoomsWithAI(userPrompt: string) {
    try {
      ...
      const requirements = await geminiService.parseRoomRequest(userPrompt);
      ...
      if (requirements.capacity) {
        ...
        filteredRooms = filteredRooms.filter((room: any) => room.capacity >= requirements.capacity!);
      ...
      if (requirements.roomType) {
        ...
        filteredRooms = filteredRooms.filter((room: any) => room.roomType === requirements.roomType);
      ...
      if (requirements.hasAirConditioner === true) {
        ...
        filteredRooms = filteredRooms.filter((room: any) => room.hasAirConditioner);
      ...
      if (requirements.hasNoiseCancelling === true) {
        ...
        filteredRooms = filteredRooms.filter((room: any) => room.hasNoiseCancelling);
      ...
      if (requirements.hasNaturalLight === true) {
        ...
        filteredRooms = filteredRooms.filter((room: any) => room.hasNaturalLight);
      ...
      const rankedRooms = await geminiService.rankRooms(requirements, filteredRooms);
      ...
      throw new HttpError(500, 'Failed to search rooms with AI');
    }
  }
  ...
  async checkRoomAvailability(roomNumber: string, startDatetime: Date, endDatetime: Date) {
    return await reservationRepository.checkRoomAvailability(roomNumber, startDatetime, endDatetime);
  }
}
